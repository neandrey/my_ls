...
//             if (xstrtoul(p, NULL, 0, &tmp_ulong, NULL) == LONGINT_OK && 0 < tmp_ulong && tmp_ulong <= SIZE_MAX)
//             {
...
//         {
//             unsigned long int tmp_ulong;
//             if (xstrtoul(optarg, NULL, 0, &tmp_ulong, NULL) != LONGINT_OK || !(0 < tmp_ulong && tmp_ulong <= SIZE_MAX))
...
//         {
//             unsigned long int tmp_ulong;
//             if (xstrtoul(optarg, NULL, 0, &tmp_ulong, NULL) != LONGINT_OK || SIZE_MAX < tmp_ulong)
...
//     if (color_indicator[C_LINK].len == 6 && !strncmp(color_indicator[C_LINK].string, "target", 6))
...
//         || ((print_inode || format_needs_type) && (type == symbolic_link || type == unknown) && (dereference == DEREF_ALWAYS || (command_line_arg && dereference != DEREF_NEVER) || color_symlink_as_referent || check_symlink_color))
...
//     bool used_color_this_time = (print_with_color && (print_color_indicator(f, symlink_target) || is_colored(C_NORM)));
...
//             type = ((!linkok && (!strncmp(color_indicator[C_LINK].string, "target", 6) || color_indicator[C_ORPHAN].string))
//                         ? C_ORPHAN
//                         : C_LINK);
